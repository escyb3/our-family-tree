...
(() => {
  ...
  function showMessage(msg) {
    ...
    // במקום alert(), היינו משתמשים במודל מותאם אישית
  }

  function showPrompt(msg) {
    // במקום prompt(), היינו משתמשים במודל מותאמת אישית
    return prompt(msg);
  }
  ...
  function start() {
    ...
    document.getElementById('add-tag')?.addEventListener('click', () => {
      ...
      if (!name) return;
      const color = showPrompt('צבע רקע (hex או מילים):', '#ffefc2') || '#ffefc2';
    ...
    document.getElementById('tags-container')?.addEventListener('click', async e => {
      if (e.target.classList.contains('apply-tag')) {
        ...
        const id = showPrompt('הכנס ID של ההודעה להחלת תג (או בחר מממשק):');
        if (!id) {
          showMessage('אין id');
          return;
        }
    ...
    document.getElementById('forward-btn')?.addEventListener('click', () => {
      ...
      const to = showPrompt('למי להעביר? (user@family.local)');
      if (!to) return;
      const msg = messages.find(m => String(m.id) === String(id));
      fetch('/api/send', {
        method: 'POST',
    ...
    function getCurrentPreviewId() {
      const id = previewSubject.dataset.id;
      return id || null;
    }
